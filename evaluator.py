...
def metric_compute(prediction, ground_truth, mask, dataset, eva_type, report=False):
...
def evaluate(prediction, ground_truth, dataset, eva_type, report=False):
    assert ground_truth.shape == prediction.shape, 'shape mis-match'
    mask_list = []
    wo_mask_list = []
    for data_idx in range(prediction.shape[1]):
        msk = np.array(ground_truth[:, data_idx] > -1e-4, dtype=np.float32)
        wo_msk = np.array(ground_truth[:, data_idx] > -2e-2, dtype=np.float32)
        mask_list.append(msk)
        wo_mask_list.append(wo_msk)
    mask_list = np.array(mask_list)
    wo_mask_list = np.array(wo_mask_list)

    performance = metric_compute(prediction, ground_truth, mask_list.T, dataset, eva_type)
    performance_wo_msk = metric_compute(prediction, ground_truth, wo_mask_list.T, dataset, eva_type)
