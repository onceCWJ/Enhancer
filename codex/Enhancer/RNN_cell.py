...
class GCRU(torch.nn.Module):
    ...
    def forward(self, inputs, hx, adj):
        ...
        :param inputs: (B, num_nodes * price_dim)
    ...
    def _fc(self, inputs, invar, market, state, output_size, bias_start=0.0):
        batch_size = inputs.shape[0]
        inputs = torch.reshape(inputs, (batch_size * self._num_nodes, -1))
        state = torch.reshape(state, (batch_size * self._num_nodes, -1))
        market = torch.reshape(market.unsqueeze(dim=1).repeat(1, self._num_nodes, 1, 1), (batch_size, self._num_nodes, -1))
        inputs_and_state = torch.cat([inputs, state, market], dim=2)
    ...
    def _gconv(self, inputs, adj_mx, state, output_size, bias_start=1.0):
        # Reshape input and state to (batch_size, num_nodes, price_dim/state_dim)
        batch_size = inputs.shape[0]
        ...
        weight = self._gconv_params.get_weights((input_size * num_matrices, output_size))
        x = torch.matmul(x, weight)  # (batch_size * self._num_nodes, output_size)
